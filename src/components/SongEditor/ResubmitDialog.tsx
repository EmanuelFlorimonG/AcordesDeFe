 ...
 */
export const ResubmitDialog: React.FC<ResubmitDialogProps> = ({ draft, sender, trackingCode, editToken, drafts, draftKey, onSent, onClose }) => {
  ...
  const send = async () => {
    ...
    setError('');
    try {
      const receipt = await sender.repository.resubmit({ trackingCode, editToken, song }, { humanCheck });
      ...
      setError(`${message} Tus cambios siguen guardados en este navegador.`);
    } finally {
